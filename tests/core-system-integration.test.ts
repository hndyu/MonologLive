...
import { ConversationContext, Comment } from '../src/types/core';
...
describe('Core System Integration - Complete User Journey', () => {
  ...
  test('Comment system generates diverse roles and handles interactions', async () => {
    ...
    const testContexts = [
      ...
      { recentTranscript: 'たしかにそうですね', currentTopic: 'agreement', userEngagement: 'medium' }
    ];
    ...
    if (commentElement) {
      ...
      const thumbsUpBtn = commentElement.querySelector('.thumbs-up-btn') as HTMLElement;
      if (thumbsUpBtn) {
        thumbsUpBtn.click();
      }
      
      // Test thumbs down if available
      const thumbsDownBtn = commentElement.querySelector('.thumbs-down-btn') as HTMLElement;
      if (thumbsDownBtn) {
        thumbsDownBtn.click();
      }
    }
  ...
  test('Component integration and data flow', async () => {
    ...
    await storage.saveUserPreferences('test_user', { roleWeights: new Map(), topicPreferences: [], interactionHistory: [], sessionCount: 1 });
