...
import { performanceMonitor, PerformanceTimer } from '../src/performance/performance-monitor';
...
describe('Performance Stress Tests', () => {
  ...
  test('High-frequency comment generation stress test', async () => {
    ...
    for (let i = 0; i < commentCount; i++) {
      ...
      const context: ConversationContext = {
        ...
        userEngagement: ['low', 'medium', 'high'][i % 3] as any,
        sessionDuration: i + 1,
        commentHistory: comments.slice(-5) // Keep last 5 comments for context
      };
      ...
      if (i > 0 && i % 50 === 0) {
        const recentAverage = performanceData.slice(-50).reduce((a, b) => a + b, 0) / 50;
        console.log(`Average generation time at ${i}: ${recentAverage.toFixed(2)}ms`);
  ...
  test('Memory usage under sustained load', async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await storage.saveUserPreferences(`user_${i % 10}`, {
        roleWeights: new Map([['reaction', 1.2], ['agreement', 0.8]]),
        topicPreferences: [`topic_${i % 5}`],
        interactionHistory: [
          { commentId: comment.id, type: 'pickup', timestamp: new Date(), confidence: 0.8 }
        ],
        sessionCount: i + 1
      });
      ...
      if (i % 50 === 0) {
        ...
        console.log(`Memory at iteration ${i}: ${metrics.memoryUsage.percentage.toFixed(1)}%`);
    ...
    const finalMemory = memorySnapshots[memorySnapshots.length - 1]?.memoryPercentage || 0;
    const memoryGrowth = finalMemory - initialMemory;

    console.log(`Memory growth: ${memoryGrowth.toFixed(1)}% (from ${initialMemory.toFixed(1)}% to ${finalMemory.toFixed(1)}%)`);
  ...
  test('Concurrent operations stress test', async () => {
    ...
    console.log(`Starting concurrent stress test: ${concurrentOperations} batches of ${operationsPerBatch} operations`);
    ...
    for (let batch = 0; batch < concurrentOperations; batch++) {
      ...
      for (let op = 0; op < operationsPerBatch; op++) {
        ...
        // Storage operation
        batchPromises.push(
          storage.saveUserPreferences(`concurrent_user_${operationId}`, {
            roleWeights: new Map(),
            topicPreferences: [],
            interactionHistory: [],
            sessionCount: 1
        ...
        batchPromises.push(
          new Promise((resolve) => {
            performanceMonitor.recordResponseTime('commentGeneration', Math.random() * 100);
            resolve(true);
          })
        );
      }
  ...
  test('Adaptive behavior under extreme stress', async () => {
    ...
    const stressConditions = [
      { responseTime: 2000, memoryUsage: 85, description: 'High response time and memory' },
      { responseTime: 1500, memoryUsage: 75, description: 'High response time' },
      { responseTime: 800, memoryUsage: 90, description: 'Critical memory usage' },
      { responseTime: 3000, memoryUsage: 80, description: 'Very high response time' }
    ];
    ...
    for (const condition of stressConditions) {
      ...
      performanceMonitor.recordResponseTime('commentGeneration', condition.responseTime);
      ...
      const optimizationStatus = adaptiveBehaviorManager.getOptimizationStatus();

      console.log(`Adaptive response: ${optimizationStatus.level} optimization, ${optimizationStatus.activeOptimizations.length} optimizations`);
    ...
    for (let i = 0; i < operationCount; i++) {
      ...
      const preferences = {
        ...
        interactionHistory: Array.from({ length: 10 }, (_, j) => ({
          ...
          type: ['pickup', 'thumbs_up', 'thumbs_down'][j % 3] as any,
          timestamp: new Date(),
      ...
      if (i > 0 && i % 20 === 0) {
        const recentAverage = storageTimes.slice(-20).reduce((a, b) => a + b, 0) / 20;
        console.log(`Average storage time at ${i}: ${recentAverage.toFixed(2)}ms`);
  ...
  test('System recovery after resource exhaustion', async () => {
    ...
    try {
      for (let i = 0; i < 1000; i++) {
        ...
        if (i % 100 === 0) {
          const metrics = performanceMonitor.getMetrics();
          console.log(`Memory at ${i} objects: ${metrics.memoryUsage.percentage.toFixed(1)}%`);
      ...
      console.log('Resource exhaustion reached:', error.message);
    }
    ...
    const recoveryComment = commentSystem.generateComment({
    ...
    await storage.saveUserPreferences('recovery_user', {
      roleWeights: new Map(),
      topicPreferences: [],
      interactionHistory: [],
      sessionCount: 1
    ...
    const recoveredPreferences = await storage.getUserPreferences('recovery_user');
