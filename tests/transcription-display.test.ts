        ...
        (transcriptEvents, config) => {
          ...
          segments.forEach((segment, index) => {
            ...
            if (segment.isFinal) {
              ...
              expect(element.classList.contains('transcript-interim')).toBe(false);
            } else {
              expect(element.classList.contains('transcript-interim')).toBe(true);
              expect(element.classList.contains('transcript-final')).toBe(false);
            }
          });

          // Property: Timestamps should be shown if configured
          const timestampElements = container.querySelectorAll('.transcript-timestamp');
          ...
          expect(contentDiv?.getAttribute('aria-label')).toBe('Live transcription');
      ...
      fc.property(
        fc.array(
          fc.oneof(
            ...
            fc.stringOf(fc.constantFrom('!', '@', '#', '$', '%', '^', '&', '*')), // Special characters
          ),
      ...
      fc.property(
        ...
        (config, texts) => {
          ...
          domSegments.forEach(element => {
            expect(element.classList.contains(config.finalTextClass)).toBe(true);
          });
          ...
          expect(updatedSegments.length).toBeLessThanOrEqual(newConfig.maxLines);
          
          const updatedTimestamps = container.querySelectorAll('.transcript-timestamp');
    ...
    fc.assert(
      fc.property(
        ...
        (actions) => {
          ...
          const interimElements = container.querySelectorAll('.transcript-interim');
