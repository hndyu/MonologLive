...
import { HybridCommentGenerator, DEFAULT_HYBRID_CONFIG } from '../src/comment-generation/hybrid-generator';
...
jest.mock('../src/comment-generation/webllm-processor', () => {
  return {
    WebLLMProcessor: jest.fn().mockImplementation(() => ({
      ...
      generateContextualComment: jest.fn().mockImplementation(async (context, role) => {
  ...
  test('Property 9: Cost-Effective Processing - No Cloud API Calls During Sessions', () => {
    fc.assert(
      fc.property(
        ...
        fc.record({
          ruleBasedRatio: fc.float({ min: Math.fround(0.5), max: Math.fround(0.9) }),
        ...
        async (contexts: ConversationContext[], ratioConfig) => {
          ...
          const usedBothMethods = metrics.ruleBasedCount > 0 || metrics.llmCount >= 0;
          ...
          const averageTimePerComment = generatedComments.length > 0 
          ...
          const actualRuleBasedRatio = totalGenerated > 0 
            ? metrics.ruleBasedCount / totalGenerated 
            : 1.0;
          
          // Allow some variance due to adaptive behavior and fallbacks
          const ratioWithinBounds = actualRuleBasedRatio >= (config.ruleBasedRatio - 0.2);
          
          // Property 4: System should maintain baseline activity (Requirement 9.3)
          const maintainedActivity = generatedComments.length > 0 || contexts.length === 0;
          
          // Property 5: LLM failures should fallback to rule-based (cost control)
          const hasGracefulFallback = config.fallbackToRuleBased ? 
            (metrics.llmFailureCount === 0 || metrics.ruleBasedCount > 0) : true;
          ...
          const hasPerformanceMonitoring = 
            ...
            metrics.llmSuccessRate >= 0 && metrics.llmSuccessRate <= 1;
          
          await generator.destroy();
          
          return usedBothMethods && 
    ...
    fc.assert(
      fc.property(
        ...
        async (context: ConversationContext, iterations: number) => {
          ...
          const ratiosChanged = finalConfig.ruleBasedRatio !== initialRuleBasedRatio;
          
          // Property 2: Poor performance should increase rule-based ratio
          const poorPerformance = finalMetrics.llmSuccessRate < 0.8 || 
                                 finalMetrics.llmAverageResponseTime > config.performanceThreshold;
          
          const appropriateAdaptation = !poorPerformance || 
      ...
      fc.property(
        fc.array(
          fc.record({
            ...
            userEngagementLevel: fc.float({ min: Math.fround(0.3), max: Math.fround(1.0) }),
            speechVolume: fc.float({ min: Math.fround(0.3), max: Math.fround(1.0) }),
            speechRate: fc.float({ min: Math.fround(0.8), max: Math.fround(1.5) }),
            silenceDuration: fc.float({ min: Math.fround(0), max: Math.fround(10) })
          }),
        ...
        async (contexts: ConversationContext[]) => {
          ...
          for (const context of contexts) {
            ...
            try {
              ...
              console.warn('Generation error (should be handled by fallback):', error);
            }
          }
          ...
          const serviceAvailability = successfulGenerations > 0 || totalAttempts === 0;
          
          // Property 2: Fallback should be used when LLM fails
          const fallbackUsed = metrics.llmFailureCount === 0 || metrics.ruleBasedCount > 0;
  ...
  test('Property: Resource Usage Remains Efficient Throughout Sessions', () => {
    fc.assert(
      fc.property(
        ...
        fc.record({
          ruleBasedRatio: fc.float({ min: Math.fround(0.6), max: Math.fround(0.9) }),
