...
import { AudioFile, AudioQualitySettings, AudioFormat } from '../src/types/core';
...
describe('Audio Recording and Storage Properties', () => {
  ...
  beforeEach(() => {
    ...
    mockDbWrapper = {
      ...
      getStorageEstimate: jest.fn().mockResolvedValue({ usage: 1000000, quota: 100000000 }),
    ...
    fc.assert(
      fc.property(
        ...
        async (qualitySettings: AudioQualitySettings, sessionId: string) => {
          ...
          if (validAudioFile) {
            const fileId = await audioManager.saveAudioFile(audioFile, sessionId);
      ...
      fc.property(
        ...
        fc.array(
          fc.record({
            ...
            format: fc.constantFrom('webm' as AudioFormat, 'mp4' as AudioFormat, 'wav' as AudioFormat),
        ...
        async (audioFiles: AudioFile[], userId: string) => {
          ...
          const userSessions = audioFiles.map(file => ({
            ...
            comments: [],
            interactions: [],
            metrics: { totalDuration: 0, commentCount: 0, interactionCount: 0, averageEngagement: 0 }
          }));
          ...
          if (audioFiles.length > 0) {
            ...
            const deleteResult = await audioManager.deleteAudioFile(fileToDelete.id);
            const deletionWorked = deleteResult === true;
            
            // Verify delete was called
            expect(mockDbWrapper.deleteAudioFile).toHaveBeenCalledWith(fileToDelete.id);
    ...
    fc.assert(
      fc.property(
        ...
        fc.record({
          ...
          totalQuota: fc.integer({ min: 100 * 1024 * 1024, max: 1000 * 1024 * 1024 }), // 100MB to 1GB
        }),
        ...
        async (storageScenario, maxAgeInDays: number) => {
          ...
          (audioManager as any).getAllAudioFiles = jest.fn().mockResolvedValue(oldFiles);
          mockDbWrapper.deleteAudioFile.mockResolvedValue(undefined);
          
          const cleanedCount = await audioManager.cleanupOldFiles(maxAgeInDays);
          const cleanupWorked = typeof cleanedCount === 'number' && cleanedCount >= 0;
  ...
  test('Property: Audio Format Support and Quality Configuration', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('webm' as AudioFormat, 'mp4' as AudioFormat, 'wav' as AudioFormat),
        fc.record({
          bitrate: fc.integer({ min: 64000, max: 320000 }),
          sampleRate: fc.constantFrom(22050, 44100, 48000),
          channels: fc.constantFrom(1, 2)
        }),
        (format: AudioFormat, quality: AudioQualitySettings) => {
          const recorder = new WebAudioRecorder();
          ...
          return configurationSucceeded && supportStatusValid && initialStatusValid;
        }
      ),
      { numRuns: 50 }
    );
