...
import { AdaptiveFrequencyManager, FrequencyAdaptationConfig, DEFAULT_FREQUENCY_CONFIG } from '../src/comment-generation/adaptive-frequency-manager';
import { AudioAnalysisData } from '../src/audio/audio-analyzer';
  ...
  test('Property 4: Volume-Based Comment Adaptation', () => {
    fc.assert(
      fc.property(
        ...
        (initialAnalysis: AudioAnalysisData, volumeSequence: AudioAnalysisData[]) => {
          const manager = new AdaptiveFrequencyManager(DEFAULT_FREQUENCY_CONFIG);
          ...
          for (const analysis of volumeSequence) {
            ...
            const currentFrequency = manager.getFrequencyState().currentFrequency;
          ...
          for (let i = 1; i < volumeSequence.length; i++) {
            ...
            const prevFreq = i === 1 ? initialFrequency : frequencyChanges[i - 2];
          ...
          const allFrequenciesInBounds = frequencyChanges.every(freq => 
          ...
          for (let i = 1; i < frequencyChanges.length; i++) {
            const change = Math.abs(frequencyChanges[i] - frequencyChanges[i - 1]);
          ...
          return volumeFrequencyCorrelation && 
                 speechRateInfluence && 
    ...
    fc.assert(
      fc.property(
        ...
        (volumeThreshold: number, analysisSequence: AudioAnalysisData[]) => {
          ...
          let belowThresholdFrequencies: number[] = [];
          let aboveThresholdFrequencies: number[] = [];
          ...
          if (belowThresholdFrequencies.length === 0 || aboveThresholdFrequencies.length === 0) {
            return true;
          }
          
          // Property: Average frequency should be lower for below-threshold volumes
          const avgBelowThreshold = belowThresholdFrequencies.reduce((sum, freq) => sum + freq, 0) / belowThresholdFrequencies.length;
          const avgAboveThreshold = aboveThresholdFrequencies.reduce((sum, freq) => sum + freq, 0) / aboveThresholdFrequencies.length;
    ...
    fc.assert(
      fc.property(
        ...
        (silenceDuration: number) => {
          ...
          const baselineFrequency = DEFAULT_FREQUENCY_CONFIG.baseFrequency * DEFAULT_FREQUENCY_CONFIG.baselineActivity;
    ...
    fc.assert(
      fc.property(
        ...
        (targetFrequency: number, numChecks: number) => {
          ...
          return actualGenerations >= expectedGenerations && actualGenerations <= maxExpectedGenerations;
        }
      ),
      { numRuns: 50 }
    );
    ...
    fc.assert(
      fc.property(
        ...
        (configParams, analysisSequence: AudioAnalysisData[]) => {
          ...
          const minFreq = Math.min(configParams.minFrequency, configParams.maxFrequency - 1);
          const maxFreq = Math.max(configParams.maxFrequency, configParams.minFrequency + 1);
          const baseFreq = Math.max(minFreq, Math.min(maxFreq, configParams.baseFrequency));
