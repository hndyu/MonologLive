...
import { PreferenceLearningSystem, DEFAULT_PREFERENCE_LEARNING_CONFIG } from '../src/learning/preference-learning';
import { IndexedDBWrapper } from '../src/storage/indexeddb-wrapper';
import { CommentRoleType, UserInteractionType } from '../src/types/core';
      ...
      fc.property(
        ...
        fc.array(
          fc.record({
            ...
            interactionType: fc.constantFrom('thumbs_up' as UserInteractionType, 'thumbs_down' as UserInteractionType, 'click' as UserInteractionType, 'pickup' as UserInteractionType),
            confidence: fc.float({ min: Math.fround(0.1), max: Math.fround(1.0) })
          }),
          { minLength: 1, maxLength: 10 }
        ),
        async (userId: string, interactions: Array<{ role: CommentRoleType; interactionType: UserInteractionType; confidence: number }>) => {
          // Create first learning system instance (first session)
          const learningSystem1 = new PreferenceLearningSystem(mockStorage, DEFAULT_PREFERENCE_LEARNING_CONFIG);
          ...
          const learningSystem2 = new PreferenceLearningSystem(mockStorage, DEFAULT_PREFERENCE_LEARNING_CONFIG);
          ...
          const persistedWeights = learningSystem2.getPersonalizedWeights(userId);
          ...
          for (const [role, weight] of updatedWeights) {
            ...
            if (!persistedWeight || Math.abs(weight - persistedWeight) > 0.001) {
              weightsMatch = false;
              break;
            }
          }
          ...
          if (interactions.length > 0) {
            for (const [role, initialWeight] of initialWeights) {
              ...
              if (finalWeight && Math.abs(initialWeight - finalWeight) > 0.001) {
          ...
          for (const [, weight] of persistedWeights) {
            if (weight < DEFAULT_PREFERENCE_LEARNING_CONFIG.minWeight || 
  ...
  test('Property: Weight Adjustment Direction Based on Feedback Type', () => {
    fc.assert(
      fc.property(
        ...
        fc.constantFrom('thumbs_up' as UserInteractionType, 'thumbs_down' as UserInteractionType),
        fc.float({ min: Math.fround(0.5), max: Math.fround(1.0) }),
        async (userId: string, role: CommentRoleType, feedbackType: 'thumbs_up' | 'thumbs_down', confidence: number) => {
          const learningSystem = new PreferenceLearningSystem(mockStorage, DEFAULT_PREFERENCE_LEARNING_CONFIG);
          ...
          // Apply feedback
          await learningSystem.updateRoleWeights(userId, role, feedbackType, confidence);
    ...
    fc.assert(
      fc.property(
        ...
        async (userId: string, interactions: Array<{ role: CommentRoleType; interactionType: UserInteractionType; confidence: number }>) => {
          ...
          const correctEventCount = stats.totalFeedbackEvents === interactions.length;
          
          // Property 2: Average weight should be within reasonable bounds
          const reasonableAverageWeight = stats.averageWeight >= DEFAULT_PREFERENCE_LEARNING_CONFIG.minWeight && 
          ...
          const validRoles: CommentRoleType[] = ['greeting', 'departure', 'reaction', 'agreement', 'question', 'insider', 'support', 'playful'];
          const validMostPreferred = validRoles.includes(stats.mostPreferredRole);
          const validLeastPreferred = validRoles.includes(stats.leastPreferredRole);
  ...
  test('Property: User Preference Independence', () => {
    fc.assert(
      fc.property(
        fc.string({ minLength: 5, maxLength: 15 }),
        fc.string({ minLength: 5, maxLength: 15 }),
        fc.constantFrom('reaction' as CommentRoleType, 'support' as CommentRoleType, 'playful' as CommentRoleType),
        fc.constantFrom('thumbs_up' as UserInteractionType, 'thumbs_down' as UserInteractionType),
        fc.float({ min: Math.fround(0.5), max: Math.fround(1.0) }),
        async (userId1: string, userId2: string, role: CommentRoleType, feedbackType: 'thumbs_up' | 'thumbs_down', confidence: number) => {
          ...
          const user1InitialWeights = learningSystem.getPersonalizedWeights(userId1);
          const user2InitialWeights = learningSystem.getPersonalizedWeights(userId2);
          ...
          await learningSystem.updateRoleWeights(userId1, role, feedbackType, confidence);
          
          // Get updated weights
          const user1UpdatedWeights = learningSystem.getPersonalizedWeights(userId1);
          const user2UpdatedWeights = learningSystem.getPersonalizedWeights(userId2);
          ...
          const user1Changed = Math.abs(user1UpdatedWeight - user1InitialWeight) > 0.001;
          
          // Property 2: User2's weight should remain unchanged
          const user2Unchanged = Math.abs(user2UpdatedWeight - user2InitialWeight) < 0.001;
          
          return user2Unchanged && (user1Changed || 
            user1InitialWeight === DEFAULT_PREFERENCE_LEARNING_CONFIG.minWeight ||
            user1InitialWeight === DEFAULT_PREFERENCE_LEARNING_CONFIG.maxWeight);
        }
      ),
      { numRuns: 50 }
    );
    ...
    fc.assert(
      fc.property(
        ...
        fc.array(
          fc.record({
            role: fc.constantFrom('greeting' as CommentRoleType, 'departure' as CommentRoleType, 'reaction' as CommentRoleType, 'agreement' as CommentRoleType, 'question' as CommentRoleType, 'insider' as CommentRoleType, 'support' as CommentRoleType, 'playful' as CommentRoleType),
            interactionType: fc.constantFrom('thumbs_up' as UserInteractionType, 'thumbs_down' as UserInteractionType),
            confidence: fc.float({ min: Math.fround(0.5), max: Math.fround(1.0) })
          }),
          { minLength: 2, maxLength: 8 }
        ),
        async (userId: string, interactions: Array<{ role: CommentRoleType; interactionType: 'thumbs_up' | 'thumbs_down'; confidence: number }>) => {
          const learningSystem = new PreferenceLearningSystem(mockStorage, DEFAULT_PREFERENCE_LEARNING_CONFIG);
          ...
          for (const [role, defaultWeight] of defaultWeights) {
            ...
            if (modifiedWeight && Math.abs(defaultWeight - modifiedWeight) > 0.001) {
              weightsChanged = true;
              break;
            }
          }
