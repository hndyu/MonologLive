...
import { AudioFile, AudioQualitySettings } from '../src/types/core';
...
class MockAudioFile implements AudioFile {
  ...
  constructor(data: Partial<AudioFile> = {}) {
    ...
    this.sessionId = data.sessionId || `session_${Math.random().toString(36).substr(2, 9)}`;
...
const audioFileGenerator = fc.record({
...
const transcriptGenerator = fc.string({ minLength: 10, maxLength: 1000 })
  .filter(s => s.trim().length > 5)
  .map(s => s.replace(/[^\w\s.,!?]/g, ' ').trim());

describe('Enhanced Transcription Property Tests', () => {
  let whisperTranscription: any;
  ...
  describe('Property 12: Enhanced Transcription Round-trip (Optional)', () => {
    test('Device capability detection should be consistent', () => {
      fc.assert(fc.property(
        ...
        async () => {
          const capabilities1 = await DeviceCapabilityDetector.detectCapabilities();
          const capabilities2 = await DeviceCapabilityDetector.detectCapabilities();
          
          // Capabilities should be consistent across calls
          expect(capabilities1.supportsWebAssembly).toBe(capabilities2.supportsWebAssembly);
          expect(capabilities1.supportsWebGPU).toBe(capabilities2.supportsWebGPU);
          expect(capabilities1.recommendedModelSize).toBe(capabilities2.recommendedModelSize);
    ...
    test('Model compatibility check should be deterministic', () => {
      fc.assert(fc.property(
        ...
        async (modelSize: 'tiny' | 'base' | 'small' | 'medium' | 'large') => {
          const compatible1 = await DeviceCapabilityDetector.checkModelCompatibility(modelSize);
          const compatible2 = await DeviceCapabilityDetector.checkModelCompatibility(modelSize);
    ...
    test('Model configuration should update model info correctly', () => {
      fc.assert(fc.property(
        fc.record({
          modelSize: fc.constantFrom('tiny', 'base', 'small', 'medium', 'large'),
