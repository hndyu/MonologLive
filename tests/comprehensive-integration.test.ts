...
import { errorHandler, ErrorType, ErrorSeverity, createError } from '../src/error-handling/error-handler';
...
import { ConversationContext, Session, UserPreferences } from '../src/types/core';
...
describe('Comprehensive Integration Tests', () => {
  ...
  describe('Error Recovery and Fallback Mechanisms', () => {
    ...
    test('Performance degradation recovery', async () => {
      ...
      const highMemoryMetrics = {
        ...
        sessionMetrics: { duration: 300, commentsGenerated: 50, transcriptionAccuracy: 0.9 }
      };
  ...
  describe('Performance Under Various Conditions', () => {
    test('High-frequency comment generation performance', async () => {
      ...
      for (let i = 0; i < commentCount; i++) {
        ...
        performanceMonitor.recordResponseTime('commentGeneration', performance.now() - startTime);
      }
    ...
    test('Memory usage under extended session', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (i % 20 === 0) {
          ...
          console.log(`Memory usage at ${i}: ${currentMetrics.memoryUsage.percentage}%`);
    ...
    test('Adaptive behavior under stress conditions', async () => {
      ...
      const config = adaptiveBehaviorManager.getCurrentConfig();
      const recommendations = adaptiveBehaviorManager.getPerformanceRecommendations();
  ...
  describe('Integration Edge Cases', () => {
    ...
    test('Resource cleanup and memory leaks prevention', async () => {
      ...
      const memoryGrowth = finalMetrics.memoryUsage.percentage - initialMetrics.memoryUsage.percentage;
