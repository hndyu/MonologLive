    ...
    fc.assert(
      fc.property(
        ...
        (silenceConfig, preSilenceAnalysis: AudioAnalysisData) => {
          ...
          const preSilenceFrequency = manager.getFrequencyState().currentFrequency;
          ...
          const expectedBaselineFrequency = config.baseFrequency * config.baselineActivity;
          ...
          const maintainsBaseline = silenceFrequency >= expectedBaselineFrequency * 0.9; // 10% tolerance
    ...
    fc.assert(
      fc.property(
        ...
        (adaptationSmoothness: number, numUpdates: number) => {
          ...
          const respectsMinimum = frequencyChanges.every(freq => freq >= config.minFrequency);
    ...
    fc.assert(
      fc.property(
        ...
        (recoveryParams) => {
          ...
          const recoveryFrequency = manager.getFrequencyState().currentFrequency;
          ...
          const appropriateRecovery = recoveryFrequency > DEFAULT_FREQUENCY_CONFIG.baseFrequency * 0.8;
          
          // Property 3: Recovery should not exceed maximum frequency
          const respectsMaximum = recoveryFrequency <= DEFAULT_FREQUENCY_CONFIG.maxFrequency;
  ...
  test('Property: Configurable Baseline Activity Maintenance', () => {
    fc.assert(
      fc.property(
        ...
        (baselineActivity: number, baseFrequency: number, silenceDuration: number) => {
          const config: FrequencyAdaptationConfig = {
            ...DEFAULT_FREQUENCY_CONFIG,
      ...
      fc.property(
        ...
        (analysisSequence: AudioAnalysisData[]) => {
          ...
          for (const analysis of analysisSequence) {
            ...
            if (isNaN(analysis.silenceDuration) || isNaN(analysis.volume) || 
                isNaN(analysis.speechRate) || isNaN(analysis.averageVolume) || 
                isNaN(analysis.volumeVariance)) {
              continue;
            }
            ...
            if (!analysis.isSpeaking && analysis.silenceDuration > 0) {
              if (Math.abs(state.silenceDuration - analysis.silenceDuration) > 100) { // 100ms tolerance
