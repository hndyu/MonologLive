...
import { SessionManager } from '../interfaces/session-management.js';
import { SummaryGenerator } from '../interfaces/summary-generation.js';
...
export class SessionManagerImpl implements SessionManager {
  ...
  private handleSpeechActivity(session: Session, activity: ActivityEvent): void {
    // Add transcript segment if provided
    if (activity.data && activity.data.transcript) {
      const segment: TranscriptSegment = {
        start: activity.timestamp.getTime() - session.startTime.getTime(),
        end: (activity.timestamp.getTime() - session.startTime.getTime()) + (activity.duration || 0),
  ...
  private handleCommentActivity(session: Session, activity: ActivityEvent): void {
    if (activity.data && activity.data.comment) {
      session.comments.push(activity.data.comment);
      session.metrics.commentCount++;
    }
  }

  private handleInteractionActivity(session: Session, activity: ActivityEvent): void {
    if (activity.data && activity.data.interaction) {
      session.interactions.push(activity.data.interaction);
      session.metrics.interactionCount++;
    }
  }

  private handleSilenceActivity(session: Session, activity: ActivityEvent): void {
  ...
  private updateSessionMetrics(session: Session): void {
    ...
    session.metrics.averageEngagement = durationMinutes > 0 
