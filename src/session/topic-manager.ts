...
import { Comment, ConversationContext } from '../types/core';
...
export class TopicManager {
   ...
   */
  setTopic(topic: string | null, source: 'user_input' | 'auto_detection' | 'system' = 'user_input'): void {
    ...
    if (previousTopic !== normalizedTopic) {
      ...
      console.log(`Topic changed from "${previousTopic}" to "${normalizedTopic}" (source: ${source})`);
    }
  }
  ...
  generateTopicBasedStarters(count: number = 3): Comment[] {
    return this.conversationStarter.generateStarterOptions(this.currentTopic || undefined, count);
  }
   ...
   * Implements Requirement 5.5
   */
  influenceCommentContext(baseContext: ConversationContext): ConversationContext {
    ...
    if (this.currentTopic) {
      ...
      if (currentProgression) {
        ...
        const topicDuration = Date.now() - currentProgression.startTime.getTime();
        if (topicDuration > 300000) { // 5 minutes
  ...
  processSpeechInput(transcript: string): void {
    ...
    if (this.currentTopic && this.config.trackTopicProgression) {
      ...
      if (currentProgression) {
        ...
        currentProgression.userEngagement += Math.min(transcript.length / 100, 2);
      }
    }
  ...
  private endCurrentTopicProgression(): void {
    ...
    if (currentProgression) {
      ...
      currentProgression.duration = currentProgression.endTime.getTime() - currentProgression.startTime.getTime();
    }
  }
  ...
  private getAverageTopicDuration(): number {
    const completedProgressions = this.topicProgression.filter(p => p.duration);
    if (completedProgressions.length === 0) return 0;
    
    const totalDuration = completedProgressions.reduce((sum, p) => sum + (p.duration || 0), 0);
