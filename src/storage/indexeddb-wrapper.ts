...
import { Session, UserPreferences, AudioFile, SessionSummary } from '../types/core.js';
...
export class IndexedDBWrapper {
  ...
  async initialize(): Promise<void> {
    this.db = await openDB<MonologDB>(this.dbName, this.version, {
      upgrade(db) {
        ...
        // Summaries store
        const summaryStore = db.createObjectStore('summaries', { keyPath: 'sessionId' });
  ...
  async clearAllData(): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    const tx = this.db.transaction(['sessions', 'preferences', 'audioFiles', 'summaries'], 'readwrite');
    await Promise.all([
