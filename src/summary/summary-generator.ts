...
export class SummaryGeneratorImpl implements SummaryGenerator {
  ...
  async createSummary(session: Session): Promise<SessionSummary> {
    ...
    // Generate insights
    const insights = await this.insightGenerator.generateInsights(topics, session);
    
    // Create overall summary
    const overallSummary = this.generateOverallSummary(session, topics, insights);
...
export class BasicTopicExtractor implements TopicExtractor {
  private commonWords = new Set([
    ...
    'very', 'really', 'quite', 'pretty', 'so', 'too', 'much', 'many', 'more', 'most',
    'just', 'only', 'even', 'also', 'still', 'yet', 'already', 'again',
    'up', 'down', 'out', 'off', 'over', 'under', 'above', 'below', 'through', 'between',
    'into', 'onto', 'from', 'about', 'around', 'across', 'along', 'against'
  ]);
  ...
  async extractTopics(text: string): Promise<Topic[]> {
    ...
    for (const [word, frequency] of wordFreq.entries()) {
      if (!this.commonWords.has(word.toLowerCase()) && word.length > 3 && frequency > 1) {
...
export class BasicInsightGenerator implements InsightGenerator {
  async generateInsights(topics: Topic[], sessionData: Session): Promise<Insight[]> {
    const insights: Insight[] = [];
    ...
    if (durationMinutes > 30) {
      insights.push({
        type: 'session_length',
        content: 'You had an extended conversation session, showing deep engagement',
        confidence: 0.9
      });
    } else if (durationMinutes < 5) {
      insights.push({
        type: 'session_length',
        content: 'This was a brief session - consider longer conversations for better insights',
        confidence: 0.8
      });
    }
    ...
    if (interactionRate > 2) {
      insights.push({
        type: 'engagement',
        content: 'High interaction rate suggests you were very engaged with the comments',
        confidence: 0.85
      });
    } else if (interactionRate < 0.5) {
      insights.push({
        type: 'engagement',
        content: 'Low interaction rate - you might prefer more independent conversation',
        confidence: 0.75
      });
    }
    ...
    if (highRelevanceTopics.length > 5) {
      ...
      insights.push({
        type: 'topic_focus',
        content: 'You maintained focus on specific topics throughout the conversation',
        confidence: 0.85
      });
    }

    // Analyze sentiment patterns
    const averageSentiment = topics.reduce((sum, topic) => sum + topic.sentiment, 0) / topics.length;
  ...
  async analyzeEmotionalTone(transcript: string): Promise<number> {
    ...
    const negativeWords = ['sad', 'angry', 'frustrated', 'terrible', 'awful', 'hate', 'disappointed'];
