...
export class PerformanceMonitor {
	...
	recordResponseTime(
		operation: keyof PerformanceMetrics["responseTime"],
		duration: number,
	): void {
	...
	recordResourceLoading(
		resource: keyof PerformanceMetrics["resourceLoading"],
		duration: number,
	): void {
	...
	updateSessionMetrics(
		updates: Partial<PerformanceMetrics["sessionMetrics"]>,
	): void {
	...
	private checkThresholds(): void {
		...
		Object.entries(responseTime).forEach(([operation, time]) => {
			if (time > this.thresholds.responseTimeCritical) {
				this.triggerWarning(
					`Critical response time for ${operation}`,
					this.metrics,
				);
			} else if (time > this.thresholds.responseTimeWarning) {
				this.triggerWarning(
					`Slow response time for ${operation}`,
					this.metrics,
				);
			}
		});
	...
	getRecommendations(): string[] {
		...
		if (memoryUsage.percentage > this.thresholds.memoryWarning) {
			recommendations.push(
				"Consider reducing memory usage by clearing unused data",
			);
		...
		if (responseTime.commentGeneration > this.thresholds.responseTimeWarning) {
			recommendations.push(
				"Consider reducing LLM usage ratio for faster comment generation",
			);
		}

		if (responseTime.transcription > this.thresholds.responseTimeWarning) {
			recommendations.push(
				"Consider using Web Speech API instead of enhanced transcription",
			);
		}
