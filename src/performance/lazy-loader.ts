...
export class LazyLoader {
  ...
  private registerDefaultFeatures(): void {
    ...
    this.registerFeature({
      ...
      isLoading: false,
      loader: async () => {
        const { WebLLMProcessor } = await import('../comment-generation/webllm-processor.js');
    ...
    this.registerFeature({
      ...
      isLoading: false,
      loader: async () => {
        const { EnhancedTranscription } = await import('../transcription/enhanced-transcription.js');
    ...
    this.registerFeature({
      ...
      isLoading: false,
      loader: async () => {
        const { PreferenceLearning } = await import('../learning/preference-learning.js');
    ...
    this.registerFeature({
      ...
      isLoading: false,
      loader: async () => {
        const { SummaryGenerator } = await import('../summary/summary-generator.js');
  ...
  private async performLoad(feature: LazyLoadableFeature): Promise<any> {
    const startTime = performance.now();
    try {
      const result = await feature.loader();
      const loadTime = performance.now() - startTime;
      console.log(`Feature '${feature.name}' loaded in ${loadTime.toFixed(2)}ms`);
      return result;
    } catch (error) {
      const loadTime = performance.now() - startTime;
      console.error(`Feature '${feature.name}' failed to load after ${loadTime.toFixed(2)}ms:`, error);
      throw error;
    }
  }
  ...
  private getCachedFeature(featureName: string): any {
  ...
  async preloadFeatures(featureNames: string[], priority: 'high' | 'medium' | 'low' = 'medium'): Promise<void> {
  ...
  async loadFeaturesConditionally(): Promise<void> {
    ...
    if (capabilities.canHandleLocalLLM) {
      await this.loadFeature('webllm').catch(error => {
        console.warn('WebLLM not available, continuing without local LLM:', error);
      });
    }
  ...
  private assessDeviceCapabilities(): {
    ...
    return {
      ...
      canHandleEnhancedTranscription: hasEnoughMemory && hasWebAssembly && hasWebWorkers
    };
  }
  ...
  getFeatureStatus(): { name: string; isLoaded: boolean; isLoading: boolean }[] {
