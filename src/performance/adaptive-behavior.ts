...
export class AdaptiveBehaviorManager {
	...
	private adaptToPerformance(metrics: PerformanceMetrics): void {
		...
		if (metrics.memoryUsage.percentage > 85) {
			...
			if (newConfig.memoryOptimizationLevel !== "aggressive") {
				...
				newConfig.commentFrequency = Math.max(
					5,
					newConfig.commentFrequency * 0.7,
				);
			...
			if (newConfig.memoryOptimizationLevel !== "moderate") {
				...
				newConfig.commentFrequency = Math.max(
					8,
					newConfig.commentFrequency * 0.85,
				);
				configChanged = true;
			}
			...
			if (newConfig.memoryOptimizationLevel !== "none") {
				...
				newConfig.llmUsageRatio = Math.min(
					this.baselineConfig.llmUsageRatio,
					newConfig.llmUsageRatio * 1.2,
				);
				newConfig.commentFrequency = Math.min(
					this.baselineConfig.commentFrequency,
					newConfig.commentFrequency * 1.15,
				);
				configChanged = true;
			}
		}
		...
		if (
			metrics.responseTime.transcription > 500 &&
			newConfig.transcriptionQuality === "enhanced"
		) {
			newConfig.transcriptionQuality = "basic";
			configChanged = true;
		}
	...
	private handlePerformanceWarning(
		warning: string,
	...
	private adaptToOfflineMode(): void {
		...
		const offlineConfig: AdaptiveConfig = {
			...
			audioRecordingEnabled:
				offlineManager.isFeatureAvailable("audioRecording"),
	...
	getPerformanceRecommendations(): string[] {
		...
		if (metrics.memoryUsage.percentage > 70) {
			...
			if (this.currentConfig.transcriptionQuality === "enhanced") {
				recommendations.push(
					"Switch to basic transcription to improve performance.",
				);
			}
		}
	...
	getOptimizationStatus(): {
		...
		if (this.currentConfig.memoryOptimizationLevel !== "none") {
			optimizations.push(
				`Memory optimization: ${this.currentConfig.memoryOptimizationLevel}`,
			);
		}
		...
		if (
			this.currentConfig.commentFrequency < this.baselineConfig.commentFrequency
		) {
			optimizations.push("Reduced comment frequency");
		}

		if (
			this.currentConfig.transcriptionQuality === "basic" &&
			this.baselineConfig.transcriptionQuality === "enhanced"
		) {
			optimizations.push("Basic transcription mode");
		}

		if (
			!this.currentConfig.audioRecordingEnabled &&
			this.baselineConfig.audioRecordingEnabled
		) {
