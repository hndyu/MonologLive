...
import { LocalLLMProcessor, ModelInfo, CommentRole } from '../interfaces/comment-generation';
import { ConversationContext } from '../types/core';

// WebLLM types and imports
interface WebLLMEngine {
  reload(modelId: string, chatOpts?: any): Promise<void>;
  chat(messages: any[], genConfig?: any): Promise<string>;
...
async function createWebLLMEngine(options?: CreateWebLLMEngineOptions): Promise<WebLLMEngine> {
  try {
...
export class WebLLMProcessor implements LocalLLMProcessor {
  ...
  private async _loadModelInternal(modelId?: string): Promise<boolean> {
    try {
      ...
      if (!this.SUPPORTED_MODELS.includes(selectedModel)) {
        console.warn(`Unsupported model: ${selectedModel}, falling back to ${this.DEFAULT_MODEL}`);
   ...
   */
  async generateContextualComment(context: ConversationContext, role: CommentRole): Promise<string> {
    if (!this.engine || !this.modelInfo.isLoaded) {
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: 'You are generating live stream comments in Japanese. Keep responses short (1-10 words), natural, and appropriate for the given role. Respond only with the comment text, no explanations.'
        },
        {
          role: 'user',
          content: prompt
   ...
   */
  private createPrompt(context: ConversationContext, role: CommentRole): string {
    const { recentTranscript, currentTopic, emotionalTone, speakingRate } = context;
    ...
    switch (role.type) {
      case 'greeting':
        prompt += 'Generate a welcoming greeting comment like "こんばんは！" or "初見です"';
        break;
      case 'departure':
      ...
      case 'question':
        ...
        break;
      case 'insider':
        prompt += 'Generate a regular viewer comment like "いつもの" or "出たｗ"';
        break;
      case 'support':
        prompt += 'Generate supportive comment like "無理しないでね" or "応援してます"';
        break;
      case 'playful':
        prompt += 'Generate playful teasing like "今の伏線？" or "台本ですか？"';
        break;
    }
