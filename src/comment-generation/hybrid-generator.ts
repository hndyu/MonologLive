...
import { CommentGenerator, CommentRole } from '../interfaces/comment-generation';
...
export class HybridCommentGenerator implements CommentGenerator {
  ...
  constructor(
    ...
    customRoleWeights?: Partial<Record<import('../types/core').CommentRoleType, number>>
  ) {
    this.config = config;
  ...
  private async initializeLLM(): Promise<void> {
    try {
      if (this.llmProcessor.isAvailable()) {
        ...
        console.warn('WebLLM not available in this environment, using rule-based only');
      }
    } catch (error) {
   ...
   */
  async generateComment(context: ConversationContext): Promise<Comment> {
   ...
   */
  private async generateLLMComment(context: ConversationContext): Promise<Comment> {
    ...
    while (retries <= this.config.maxLLMRetries) {
      try {
        ...
        const content = await this.llmProcessor.generateContextualComment(context, selectedRole);
  ...
  private adjustRatiosBasedOnPerformance(): void {
    ...
    if (llmSuccessRate < 0.8 || llmAverageResponseTime > this.config.performanceThreshold) {
      this.config.ruleBasedRatio = Math.min(0.9, this.config.ruleBasedRatio + 0.1);
      this.config.llmRatio = 1.0 - this.config.ruleBasedRatio;
      
      console.log(`Adjusted ratios due to poor LLM performance: Rule-based ${this.config.ruleBasedRatio}, LLM ${this.config.llmRatio}`);
    }
    
    // Decrease rule-based ratio if LLM performance is excellent
    else if (llmSuccessRate > 0.95 && llmAverageResponseTime < this.config.performanceThreshold * 0.5) {
      this.config.ruleBasedRatio = Math.max(0.5, this.config.ruleBasedRatio - 0.05);
      this.config.llmRatio = 1.0 - this.config.ruleBasedRatio;
      
      console.log(`Adjusted ratios due to excellent LLM performance: Rule-based ${this.config.ruleBasedRatio}, LLM ${this.config.llmRatio}`);
    }
  }
  ...
  private updateLLMMetrics(success: boolean, responseTime: number): void {
    if (success) {
      ...
      this.metrics.llmSuccessRate = 0.9 * this.metrics.llmSuccessRate + 0.1 * 1.0;
      
      // Update average response time
      this.metrics.llmAverageResponseTime = 0.9 * this.metrics.llmAverageResponseTime + 0.1 * responseTime;
    } else {
      this.metrics.llmFailureCount++;
      this.metrics.llmSuccessRate = 0.9 * this.metrics.llmSuccessRate + 0.1 * 0.0;
    }
  }
   ...
   */
  private selectRoleForContext(context: ConversationContext, roles: CommentRole[]): CommentRole {
    // Simple role selection based on context
    // This could be enhanced with more sophisticated logic
    
    const { silenceDuration, emotionalTone, currentTopic } = context;
    ...
    if (emotionalTone && ['excited', 'happy', 'surprised'].includes(emotionalTone)) {
  ...
  updateRoleWeights(feedback: UserFeedback): void {
    ...
    console.log(`Updated role weights based on feedback for comment ${feedback.commentId}`);
  }
  ...
  setMixingRatio(ruleBasedRatio: number, llmRatio: number): void {
    ...
    if (total > 0) {
      ...
      console.log(`Updated mixing ratios: Rule-based ${this.config.ruleBasedRatio}, LLM ${this.config.llmRatio}`);
    }
  }
