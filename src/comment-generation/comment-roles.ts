...
import { CommentRole, TriggerCondition } from '../interfaces/comment-generation';
import { CommentRoleType } from '../types/core';
 ...
 */
export function createCommentRole(type: CommentRoleType, weight?: number): CommentRole {
  return {
    type,
 ...
 */
export function createAllCommentRoles(customWeights?: Partial<Record<CommentRoleType, number>>): CommentRole[] {
...
export class RoleSelector {
   ...
   */
  selectRoles(context: any): CommentRole[] {
   ...
   */
  private calculateRoleScore(role: CommentRole, context: any): number {
   ...
   */
  private evaluateTrigger(trigger: TriggerCondition, context: any): boolean {
   ...
   */
  private getContextValue(triggerType: string, context: any): any {
    switch (triggerType) {
      case 'keyword':
