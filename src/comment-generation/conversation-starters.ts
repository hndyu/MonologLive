...
export class ConversationStarterGenerator {
  ...
  generateConversationStarter(topic?: string): Comment {
    ...
    if (topic) {
      ...
      starters = CONVERSATION_STARTERS[selectedTopic] || CONVERSATION_STARTERS.general;
    } else {
    ...
    const availableStarters = starters.filter(starter => !this.usedStarters.has(starter));
    ...
    const selectedStarter = availableStarters[Math.floor(Math.random() * availableStarters.length)];
  ...
  generateFollowUpStarter(silenceDuration: number): Comment | null {
    ...
    const selectedStarter = followUpStarters[Math.floor(Math.random() * followUpStarters.length)];
