...
import { CommentRole } from '../interfaces/comment-generation';
import { RoleSelector, createAllCommentRoles } from './comment-roles';
...
export class RuleBasedCommentGenerator {
   ...
   */
  generateComment(context: ConversationContext): Comment | null {
   ...
   */
  private generateCommentContent(role: CommentRole, context: ConversationContext): string {
    const patterns = role.patterns;
    
    // Context-aware pattern selection
    const contextualPatterns = this.filterPatternsByContext(patterns, context);
    const availablePatterns = contextualPatterns.length > 0 ? contextualPatterns : patterns;
    
    // Avoid repeating recent comments
    const recentContents = this.commentHistory
      .slice(-5)
      .map(c => c.content);
    
    const unusedPatterns = availablePatterns.filter(p => !recentContents.includes(p));
    const finalPatterns = unusedPatterns.length > 0 ? unusedPatterns : availablePatterns;
    
    // Select random pattern
    const selectedPattern = finalPatterns[Math.floor(Math.random() * finalPatterns.length)];
   ...
   */
  private filterPatternsByContext(patterns: string[], context: ConversationContext): string[] {
   ...
   */
  private isPatternRelevant(pattern: string, transcript: string, context: ConversationContext): boolean {
    ...
    if (pattern.includes('寝る') || pattern.includes('おやすみ')) {
      return hour > 21 || hour < 6 || transcript.includes('疲れ') || transcript.includes('眠い');
    }
   ...
   */
  private processTemplate(template: string, _context: ConversationContext): string {
    // Simple template processing - can be extended
    let processed = template;
    
    // Add contextual variations
    if (Math.random() < 0.1) { // 10% chance to add emphasis
  ...
  updateRoleWeights(feedback: UserFeedback): void {
    const comment = this.commentHistory.find(c => c.id === feedback.commentId);
    if (!comment) return;
    
    const adjustment = feedback.type === 'positive' ? 0.1 : 
                      feedback.type === 'negative' ? -0.1 : 0;
  ...
  getActiveRoles(): CommentRole[] {
    return this.roleSelector['roles']; // Access private property for interface compliance
  }
  ...
  getStats(): {
    ...
    this.commentHistory.forEach(comment => {
      roleDistribution[comment.role] = (roleDistribution[comment.role] || 0) + 1;
    });
    
    const intervals = this.commentHistory
      .slice(1)
      .map((comment, index) => 
        comment.timestamp.getTime() - this.commentHistory[index].timestamp.getTime()
      );
