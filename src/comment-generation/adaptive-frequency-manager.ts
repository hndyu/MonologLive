...
export class AdaptiveFrequencyManager {
  ...
  updateFromAudioAnalysis(analysisData: AudioAnalysisData): void {
    ...
    targetFrequency += (volumeFactor - 1) * this.config.volumeMultiplier * this.config.baseFrequency;
    ...
    targetFrequency += (speechRateFactor - 1) * this.config.speechRateMultiplier * this.config.baseFrequency;
    ...
    if (this.state.isInSilence) {
      ...
      const silenceReduction = Math.min(this.config.silenceReduction, 
        this.state.silenceDuration / 5000); // gradual over 5 seconds
      const reducedFrequency = targetFrequency * (1 - silenceReduction);
      const baselineFrequency = this.config.baseFrequency * this.config.baselineActivity;
      targetFrequency = Math.max(reducedFrequency, baselineFrequency);
    }

    // Apply frequency bounds
    targetFrequency = Math.max(this.config.minFrequency, 
    ...
    this.state.currentFrequency = this.state.currentFrequency * this.config.adaptationSmoothness +
      this.state.targetFrequency * adaptationRate;

    // Track adaptation history
    this.state.adaptationHistory.push(this.state.currentFrequency);
    if (this.state.adaptationHistory.length > 60) { // Keep last minute of data
  ...
  shouldGenerateComment(): boolean {
    ...
    const timeSinceLastComment = this.state.lastCommentTime > 0 ? 
      now - this.state.lastCommentTime : 0;
  ...
  private calculateEngagementLevel(): 'low' | 'medium' | 'high' {
    const avgFrequency = this.state.adaptationHistory.length > 0 ?
      this.state.adaptationHistory.reduce((sum, freq) => sum + freq, 0) / this.state.adaptationHistory.length :
  ...
  getStats(): {
    ...
    return {
      ...
      timeSinceLastComment: this.state.lastCommentTime > 0 ? now - this.state.lastCommentTime : 0,
      isInSilence: this.state.isInSilence,
