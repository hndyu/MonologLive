...
import { Comment, UserInteraction, UserInteractionType, ConversationContext, InteractionEvent } from '../types/core.js';
...
export class InteractionTracker {
  ...
  private recentComments: Map<string, { comment: Comment; timestamp: number }> = new Map();
  private interactionHistory: InteractionEvent[] = [];
  
  constructor(storage: IndexedDBWrapper, config: InteractionTrackerConfig = DEFAULT_INTERACTION_TRACKER_CONFIG) {
   ...
   */
  detectCommentPickup(subsequentSpeech: string, speechTimestamp: Date): PickupDetectionResult[] {
    const results: PickupDetectionResult[] = [];
    const speechTime = speechTimestamp.getTime();
    
    for (const [commentId, { comment, timestamp }] of this.recentComments) {
      ...
      const timingScore = Math.max(0, 1 - (timeDiff / this.config.pickupDetectionWindow));
      
      // Calculate content similarity score
      const contentScore = this.calculateContentSimilarity(comment.content, subsequentSpeech);
      
      // Calculate overall confidence
      const confidence = (timingScore * this.config.timingWeight) + (contentScore * this.config.contentWeight);
  ...
  async trackExplicitInteraction(
    ...
    console.log(`Tracked explicit interaction: ${type} for comment ${commentId}`);
  }
  ...
  async getSessionInteractions(sessionId: string): Promise<InteractionEvent[]> {
    ...
    const memoryInteractions = this.interactionHistory.filter(event => event.sessionId === sessionId);
  ...
  getInteractionStats(): {
    ...
    const pickups = this.interactionHistory.filter(e => e.type === 'pickup').length;
    const explicit = this.interactionHistory.filter(e => e.type !== 'pickup').length;
  ...
  private trackPickupInteraction(comment: Comment, confidence: number, timestamp: Date): void {
    ...
    console.log(`Detected pickup for comment ${comment.id} with confidence ${confidence.toFixed(2)}`);
  }
   ...
   */
  private calculateContentSimilarity(commentContent: string, speech: string): number {
    ...
    const intersection = new Set([...commentWords].filter(word => speechWords.has(word)));
