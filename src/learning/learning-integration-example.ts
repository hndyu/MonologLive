...
export class LearningIntegrationExample {
  ...
  async initialize(userId: string, sessionId: string): Promise<void> {
    ...
    await this.commentSystem.initializeLearning(this.storage, userId, sessionId);
    
    // Initialize preference learning
    await this.preferenceLearning.initializePreferences(userId);
    
    console.log(`Learning system initialized for user ${userId}, session ${sessionId}`);
  }
  ...
  async demonstrateLearning(userId: string): Promise<void> {
    ...
    const context = {
      recentTranscript: 'I had a great day today, went to the park and saw some beautiful flowers',
    ...
    const initialWeights = this.preferenceLearning.getPersonalizedWeights(userId);
    ...
    await this.preferenceLearning.updateRoleWeights(userId, 'reaction', 'thumbs_up', 1.0);
    await this.preferenceLearning.updateRoleWeights(userId, 'reaction', 'pickup', 0.8);
    
    // User dislikes playful comments
    await this.preferenceLearning.updateRoleWeights(userId, 'playful', 'thumbs_down', 1.0);
    
    // User shows interest in supportive comments
    await this.preferenceLearning.updateRoleWeights(userId, 'support', 'click', 0.5);
    await this.preferenceLearning.updateRoleWeights(userId, 'support', 'pickup', 0.6);
    
    // Show updated preferences
    const updatedWeights = this.preferenceLearning.getPersonalizedWeights(userId);
    ...
    await this.commentSystem.initializeLearning(this.storage, userId, 'demo-session-2');
  ...
  showInteractionStats(): void {
    ...
    if (interactionStats) {
      ...
      console.log(`  Total interactions: ${interactionStats.totalInteractions}`);
      console.log(`  Pickup rate: ${(interactionStats.pickupRate * 100).toFixed(1)}%`);
      console.log(`  Explicit feedback rate: ${(interactionStats.explicitFeedbackRate * 100).toFixed(1)}%`);
    ...
    if (learningStats) {
      console.log('\nLearning Statistics:');
      console.log(`  Total feedback events: ${learningStats.totalFeedbackEvents}`);
      console.log(`  Average weight: ${learningStats.averageWeight.toFixed(2)}`);
