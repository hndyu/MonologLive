...
import type { LearningModule } from "../interfaces/learning.js";
...
export class MainLearningModule implements LearningModule {
	...
	constructor(
		...
		this.interactionTracker = new InteractionTracker(
			storage,
			config.interactionTracker,
		);
		this.preferenceLearning = new PreferenceLearningSystem(
			storage,
	...
	detectCommentPickup(
		...
		const results = this.interactionTracker.detectCommentPickup(
			subsequentSpeech,
			speechTimestamp,
		);
	 ...
	 */
	private handleSpeechResponse(
		comment: Comment,
		speech: string,
		timestamp: Date,
	): void {
		const confidence = this.detectCommentPickup(
			comment,
			speech,
		...
		if (
			confidence > this.config.interactionTracker.contentSimilarityThreshold
		) {
	 ...
	 */
	private handleExplicitInteraction(
		comment: Comment,
		userResponse: {
			type: "click" | "feedback";
			timestamp: Date;
		...
		const interactionType: UserInteractionType =
			userResponse.type === "click" ? "click" : "thumbs_up";
