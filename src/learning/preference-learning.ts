...
export class PreferenceLearningSystem {
	...
	constructor(
		...
		config: PreferenceLearningConfig = DEFAULT_PREFERENCE_LEARNING_CONFIG,
	) {
	...
	async updateRoleWeights(
		...
		const currentWeight =
			this.currentPreferences.roleWeights.get(commentRole) || 1.0;
		...
		const adjustment =
			feedbackMultiplier * confidence * this.config.learningRate;
	...
	getPreferenceRanking(): {
		most: CommentRoleType;
		least: CommentRoleType;
	} | null {
		if (!this.currentPreferences) {
			return null;
		}
	...
	private updateLearningStats(role: CommentRoleType, adjustment: number): void {
		...
		const currentAdjustments =
			this.learningStats.roleAdjustments.get(role) || 0;
		this.learningStats.roleAdjustments.set(
			role,
			currentAdjustments + Math.abs(adjustment),
		);

		// Update average weight
		if (this.currentPreferences) {
			const weights = Array.from(this.currentPreferences.roleWeights.values());
			this.learningStats.averageWeight =
