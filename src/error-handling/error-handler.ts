...
export class ErrorHandler {
  ...
  private errorCallbacks: Map<ErrorType, ((error: MonologError) => void)[]> = new Map();
  ...
  private getVoiceInputRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      ...
      userMessage: 'Voice input temporarily unavailable. Attempting to reconnect...',
      retryable: true
    };
  }

  private getAudioRecordingRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
      ...
      userMessage: 'Audio recording disabled. Session will continue without recording.',
      retryable: false
    };
  }

  private getCommentGenerationRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
      ...
      userMessage: 'Comment generation temporarily limited. Basic comments available.',
      retryable: true
    };
  }

  private getLocalLLMRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
      ...
      userMessage: 'AI features temporarily unavailable. Using basic comment generation.',
      retryable: false
    };
  }

  private getStorageRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
      ...
      userMessage: 'Storage temporarily unavailable. Session data will not be saved.',
      retryable: true
    };
  }

  private getSessionRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
        // Create new session
  ...
  private getTranscriptionRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
      ...
      userMessage: 'Enhanced transcription unavailable. Using standard transcription.',
      retryable: false
    };
  }

  private getNetworkRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: true,
      fallbackAction: async () => {
        // Enable offline mode
      ...
      userMessage: 'Network unavailable. Running in offline mode with limited features.',
      retryable: true
    };
  }

  private getPermissionRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: false,
      userMessage: 'Microphone permission required. Please enable microphone access and refresh.',
      retryable: false
    };
  }

  private getBrowserCompatibilityRecovery(error: MonologError): ErrorRecoveryStrategy {
    return {
      canRecover: false,
      userMessage: 'Your browser does not support required features. Please use a modern browser.',
      retryable: false
    };
  }
