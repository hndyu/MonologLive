...
import { ConversationContext } from './types/core.js';
...
export class VoiceChatIntegrationTest {
  ...
  private setupTestEnvironment(): void {
    ...
    this.testContainer = document.getElementById('integration-test') || document.createElement('div');
  ...
  private setupEventHandlers(): void {
    ...
    interactionsBtn?.addEventListener('click', () => this.testCommentInteractions());
  }
  ...
  private getTranscriptionContainer(): HTMLElement {
    return document.getElementById('transcription-container') || document.createElement('div');
  }
  ...
  private getCommentContainer(): HTMLElement {
    return document.getElementById('comment-container') || document.createElement('div');
  }

  /**
   * Logs test results to the output area
   */
  private logResult(testName: string, success: boolean, details?: string): void {
    ...
    console.log(`Integration Test - ${testName}: ${success ? 'PASS' : 'FAIL'}`, details);
  }
  ...
  async runFullIntegrationTest(): Promise<void> {
    this.logResult('Integration Test Started', true, 'Testing voice input, transcription, and comments');
    ...
    try {
      ...
      this.logResult('Integration Test Complete', passedTests === totalTests, 
  ...
  private async testVoiceInputInitialization(): Promise<void> {
    try {
      ...
      this.logResult('Voice Input Support Detection', true, 
        `Web Speech API supported: ${isSupported}`);

      // Test voice input callbacks setup
      let transcriptReceived = false;
      let errorHandled = false;

      this.voiceManager.onTranscript((text, isFinal) => {
        transcriptReceived = true;
        this.logResult('Voice Input Transcript Callback', true, 
          `Received: "${text}" (final: ${isFinal})`);
      });

      this.voiceManager.onError((error) => {
        errorHandled = true;
        this.logResult('Voice Input Error Handling', true, 
          `Error handled: ${error.error}`);
      });

      this.logResult('Voice Input Initialization', true, 'Callbacks configured successfully');
  ...
  private async testTranscriptionDisplay(): Promise<void> {
    try {
      ...
      this.logResult('Transcription Interim Display', true, 'Interim transcript added');

      // Test adding final transcript
      this.transcriptionDisplay.addTranscript('こんにちは、今日はいい天気ですね', true);
      this.logResult('Transcription Final Display', true, 'Final transcript added');

      // Test transcript retrieval
      const transcriptText = this.transcriptionDisplay.getTranscriptText();
      const hasContent = transcriptText.length > 0;
      this.logResult('Transcription Text Retrieval', hasContent, 
        `Retrieved: "${transcriptText}"`);

      // Test segment management
      const segments = this.transcriptionDisplay.getSegments();
      this.logResult('Transcription Segment Management', segments.length > 0, 
  ...
  private async testCommentSystemInitialization(): Promise<void> {
    try {
      ...
      const hasValidComment = comment && comment.content && comment.role;
      this.logResult('Comment Generation', hasValidComment, 
      ...
      this.logResult('Comment Role Weights', hasRoleWeights, 
        `${Object.keys(roleWeights).length} roles configured`);

      // Test active roles
      const activeRoles = this.commentSystem.getActiveRoles();
      this.logResult('Comment Active Roles', activeRoles.length > 0, 
      ...
      this.logResult('Comment Display Integration', visibleComments >= 0, 
  ...
  private async testEndToEndFlow(): Promise<void> {
    try {
      ...
      this.logResult('End-to-End Conversation Flow', flowSuccess, 
  ...
  private async testCommentInteractionHandling(): Promise<void> {
    try {
      ...
      const testComment = this.commentSystem.generateComment(this.conversationContext);
      
      if (testComment) {
        // Test comment click simulation (pickup detection)
        const commentElement = document.querySelector(`[data-comment-id="${testComment.id}"]`) as HTMLElement;
        if (commentElement) {
          commentElement.click();
          this.logResult('Comment Click Interaction', true, 
            `Comment ${testComment.id} clicked successfully`);
        } else {
          this.logResult('Comment Click Interaction', false, 
            'Comment element not found in DOM');
        }

        // Test thumbs up interaction
        const thumbsUpBtn = document.querySelector(`[data-comment-id="${testComment.id}"] .thumbs-up-btn`) as HTMLElement;
        if (thumbsUpBtn) {
          thumbsUpBtn.click();
          this.logResult('Thumbs Up Interaction', true, 
            `Thumbs up clicked for comment ${testComment.id}`);
        } else {
          this.logResult('Thumbs Up Interaction', false, 
            'Thumbs up button not found');
        }

        // Test thumbs down interaction
        const thumbsDownBtn = document.querySelector(`[data-comment-id="${testComment.id}"] .thumbs-down-btn`) as HTMLElement;
        if (thumbsDownBtn) {
          thumbsDownBtn.click();
          this.logResult('Thumbs Down Interaction', true, 
            `Thumbs down clicked for comment ${testComment.id}`);
        } else {
          this.logResult('Thumbs Down Interaction', false, 
            'Thumbs down button not found');
        }
      } else {
        this.logResult('Comment Interaction Setup', false, 
  ...
  async simulateSpeechInput(): Promise<void> {
    ...
    for (const phrase of testPhrases) {
      ...
      this.transcriptionDisplay.addTranscript(phrase.substring(0, phrase.length - 1), false);
    ...
    this.logResult('Speech Simulation', true, `${testPhrases.length} phrases simulated`);
  }
  ...
  async testCommentGeneration(): Promise<void> {
    const testContexts = [
      { recentTranscript: 'おはよう', currentTopic: 'greeting', userEngagement: 'high' },
      { recentTranscript: 'そろそろ寝ます', currentTopic: 'departure', userEngagement: 'low' },
      { recentTranscript: 'かわいい猫の動画見てる', currentTopic: 'entertainment', userEngagement: 'medium' },
      { recentTranscript: '今日何してた？', currentTopic: 'daily_life', userEngagement: 'medium' }
    ];
    ...
    for (const context of testContexts) {
      ...
      const isValidComment = comment && comment.content && comment.role;
      
      this.logResult(`Comment Generation - ${context.currentTopic}`, isValidComment, 
  ...
  async testCommentInteractions(): Promise<void> {
    ...
    commentElements.forEach((element, index) => {
      if (index < 2) { // Test first 2 comments
        // Simulate click
        ...
        if (index === 0) {
          const thumbsUp = element.querySelector('.thumbs-up-btn') as HTMLElement;
          if (thumbsUp) {
            thumbsUp.click();
            interactionCount++;
          }
        ...
        if (index === 1) {
          const thumbsDown = element.querySelector('.thumbs-down-btn') as HTMLElement;
          if (thumbsDown) {
            thumbsDown.click();
            interactionCount++;
          }
    ...
    this.logResult('Comment Interactions Test', interactionCount > 0, 
