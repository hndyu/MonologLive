...
import { AudioFile } from '../types/core';
import { EnhancedTranscription, TranscriptionResult } from '../interfaces/enhanced-transcription';
import { WhisperTranscription, DeviceCapabilityDetector } from './enhanced-transcription';
...
export class TranscriptionIntegration {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      if (!capabilities.supportsWebAssembly) {
        console.warn('WebAssembly not supported, enhanced transcription unavailable');
        return false;
      }
      ...
      if (whisperTranscription.isAvailable()) {
        ...
        console.log(`Enhanced transcription initialized with ${capabilities.recommendedModelSize} model`);
  ...
  async transcribeAudioFile(audioFile: AudioFile): Promise<TranscriptionResult | null> {
    if (!this.isAvailable()) {
