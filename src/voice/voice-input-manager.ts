...
import { VoiceInputManager, SpeechRecognitionError, VoiceInputConfig } from '../interfaces/voice-input.js';
...
export class WebSpeechVoiceInputManager implements VoiceInputManager {
  ...
  startListening(): void {
    if (!this.isSupported()) {
      this.handleError('not-supported', 'Web Speech API is not supported in this browser');
      return;
    }
    ...
    try {
      ...
      this.handleError('audio-capture', `Failed to start voice recognition: ${error}`);
    }
  }
  ...
  private handleRecognitionError(event: SpeechRecognitionErrorEvent): void {
    ...
    switch (errorType) {
      case 'no-speech':
        ...
        break;
      case 'audio-capture':
        this.handleError('audio-capture', 'Microphone access denied or unavailable');
        break;
      case 'not-allowed':
    ...
    this.restartTimeout = window.setTimeout(() => {
      if (this.isCurrentlyListening) {
        try {
          ...
          this.handleError('restart-failed', `Failed to restart recognition: ${error}`);
        }
      }
    }, 1000);
  }
